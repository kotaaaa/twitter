...
def get_CountVec(tweet):
    c_vec = CountVectorizer(min_df=10)     # CountVectorizerオブジェクトの生成
...
def main():
    ...
    # feature = make_tweet_bag(c_vec, All_tweet)
    # feature_with_label = set_target(feature)
    # feature_with_label.to_csv('feature_with_label.csv')
    # feature_with_label = pd.read_csv('feature_with_label.csv')
    # clf = rf_train(feature_with_label)
    # clf_save(clf, save_clf)
    # clf = clf_load(load_clf)

    sort_fi(clf, feature_with_label)
